...
export const EditarConfig = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // 5. Verificar se o novo email já está em uso por outro usuário
        ...
        if (emailCheckResult[0].count > 0) {
            return res.json({ success: false, message: 'O email já está em uso por outro usuário.' });
        }

        // 6. Montar a query de atualização de acordo com os dados fornecidos
        ...
        // 7. Se a senha será atualizada, inclui na query e no array de parâmetros
        if (senhaAtualizada) {
        ...
        // 8. Executar a query de atualização
