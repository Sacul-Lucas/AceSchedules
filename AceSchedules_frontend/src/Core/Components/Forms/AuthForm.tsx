import { Link } from "react-router-dom";
import { IMaskInput } from "react-imask";
import { ChangeEventHandler, FormEventHandler, SetStateAction, useState } from "react";
import authStyles from "../../Css/Owned/Auth.module.css";
import 'primeicons/primeicons.css';

interface AuthFormProps {
    formId: string;
    formAction: ((e: { preventDefault: () => void; }) => Promise<void>) | undefined;
    formBttTitle: string;
    formMethod: string;
    userAction?: ChangeEventHandler<HTMLInputElement> | undefined;
    telAction?: FormEventHandler<HTMLInputElement> | undefined | any;
    cnpjAction?: ChangeEventHandler<HTMLInputElement> | undefined;
    emailAction: (e: { target: { value: SetStateAction<string>; }; }) => void;
    senhaAction: (e: { target: { value: SetStateAction<string>; }; }) => void;
    typeAction: FormEventHandler<HTMLLabelElement> | undefined | any;
}

export const AuthForm: React.FC<AuthFormProps> = ({
    formId,
    formAction,
    formBttTitle,
    formMethod,
    userAction,
    telAction,
    cnpjAction,
    emailAction,
    senhaAction,
    typeAction
}) => {
    const [showPassword, setShowPassword] = useState(false);

    return (
        <section className={authStyles['Auth-section']}>
            <form
                className={`${authStyles['form-box']} animate-[2s_showUp_ease-in] lg:!w-[32.8dvw] sm:!text-base backdrop-blur-[6px]`}
                id={formId}
                method={formMethod}
                onSubmit={formAction}
            >
                <div className="sticky top-0 z-auto w-full h-full">
                    <h2 className={`${authStyles['Auth-title']} lg:!text-5xl sm:!text-5xl`}>{formId}</h2>

                    <div className="!mx-5 lg:!mx-11">
                        {formId === 'Cadastro' && (
                            <div>
                                <label className={authStyles['Auth-label']} htmlFor="usuario">Empresa/Usuário</label>
                                <div className="flex flex-row w-full space-x-3">
                                    <div className="flex-1">
                                        <div className={authStyles.inputbox}>
                                            <input
                                                type="text"
                                                id="usuario"
                                                name="usuario"
                                                placeholder="Insira o nome da empresa/usuário"
                                                onChange={userAction}
                                                required
                                            />
                                        </div>
                                    </div>
                                </div>

                                <label className={authStyles['Auth-label']} htmlFor="telefone">Telefone</label>
                                <div className="flex flex-row w-full space-x-3">
                                    <div className="flex-1">
                                        <div className={authStyles.inputbox}>
                                            <IMaskInput
                                                mask="+{55} (00) 00000-0000"
                                                definitions={{ '0': /[0-9]/ }}
                                                unmask="typed"
                                                type="text"
                                                id="telefone"
                                                name="telefone"
                                                placeholder="Insira o seu telefone"
                                                onChange={telAction}
                                                required
                                            />
                                        </div>
                                    </div>
                                </div>

                                <label className={authStyles['Auth-label']} htmlFor="cnpj">CNPJ</label>
                                <div className="flex flex-row w-full space-x-3">
                                    <div className="flex-1">
                                        <div className={authStyles.inputbox}>
                                            <IMaskInput
                                                mask="00.000.000/0000-00"
                                                definitions={{ '0': /[0-9]/ }}
                                                unmask="typed"
                                                type="text"
                                                id="cnpj"
                                                name="cnpj"
                                                placeholder="Insira o seu CNPJ"
                                                onChange={cnpjAction}
                                                required
                                            />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        )}

                        <label className={authStyles['Auth-label']} htmlFor="email">
                            {formId === 'Cadastro' ? 'Email' : 'Email/Usuário'}
                        </label>
                        <div className="flex flex-row w-full space-x-3">
                            <div className="flex-1">
                                <div className={authStyles.inputbox}>
                                    <input
                                        autoComplete="on"
                                        type="text"
                                        id="email"
                                        name="email"
                                        placeholder={formId === 'Cadastro' ? 'Insira o seu email' : 'Insira o seu email/nome de usuário'}
                                        onChange={emailAction}
                                        required
                                    />
                                </div>
                            </div>
                        </div>

                        <label className={authStyles['Auth-label']} htmlFor="senha">Senha</label>
                        <div className="flex flex-row w-full space-x-3">
                            <div className="flex-1">
                                <div className={`${authStyles.inputbox} relative`}>
                                    <input
                                        autoComplete="off"
                                        type={showPassword ? "text" : "password"}
                                        id="senha"
                                        name="senha"
                                        placeholder="Insira a sua senha"
                                        onChange={senhaAction}
                                        required
                                    />
                                    <button
                                        type="button"
                                        onClick={() => setShowPassword(!showPassword)}
                                        className="absolute sm:!top-[10%] xsm:!top-[5%] right-2 text-white hover:text-white-700 focus:outline-none"
                                    >
                                        <i className={`pi ${showPassword ? 'pi-eye-slash' : 'pi-eye'}`}></i>
                                    </button>
                                </div>
                            </div>
                        </div>

                        {formId === 'Login' ? (
                            <div>
                                <div className={`pt-2 ${authStyles.forget}`}>
                                    <span>Ainda não é cadastrado? <Link to="/">Cadastre-se</Link></span>
                                </div>
                                <div className={`pt-2 ${authStyles.forget}`}>
                                    <span>Esqueceu sua senha? <Link to="/Login/Recovery">Recuperar senha</Link></span>
                                </div>
                            </div>
                        ) : (
                            <div className={`pt-2 ${authStyles.forget}`}>
                                <span>Já é cadastrado? <Link to="/Login">Entrar agora</Link></span>
                            </div>
                        )}

                        <div className="typebox">
                            <label
                                className={authStyles['Auth-label']}
                                htmlFor="types"
                                id="usertype"
                                onChange={typeAction}
                            >
                                Tipo de usuário:
                                <select name="types" id="types" className="ml-2 text-black rounded-lg">
                                    <option value="Empresa">Empresa</option>
                                    <option value="Administrador">Administrador</option>
                                </select>
                            </label>
                        </div>

                        <div className="flex items-center justify-center w-full h-auto mt-3 mb-4 align-middle">
                            <button
                                className={`${authStyles.Auth} lg:!w-[30%] lg:!text-base text-base sm:!text-lg sm:!w-[30%] xsm:!text-[0.625rem] xsm:!w-[40%]`}
                                type="submit"
                                id={formBttTitle}
                                name={formBttTitle}
                            >
                                {formBttTitle}
                            </button>
                        </div>
                    </div>
                </div>
            </form>
        </section>
    );
};
