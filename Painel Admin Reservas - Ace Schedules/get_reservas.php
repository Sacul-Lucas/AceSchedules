...
if (empty($reservas)) {
    ...
    foreach ($reservas as $row) {
        $html .= "<tr>
                    ...
                    <td>";
        if ($status == 0) {
            $html .= "<button type='button' data-id='".$row['id']."' class='viewBtn btn btn-info btn-sm'>Visualizar</button>                        
                    <button class='btn btn-success btn-sm approve-btn' data-id='" . $row['id'] . "'>Aprovar</button>
                    <button type='button' data-id='".$row['id']."' class='deleteBtn btn btn-danger btn-sm'>Rejeitar</button>               
                </td>
              </tr>";
        } else {
            $html .= "<button type='button' data-id='".$row['id']."' class='viewBtn btn btn-info btn-sm'>Visualizar</button> 
                      <button type='button' data-id='".$row['id']."' class='editBtn btn btn-success btn-sm'>Edit</button>
                    <button type='button' data-id='".$row['id']."' class='deleteBtn btn btn-danger btn-sm'>Cancelar Reserva</button> 
                    </td>
                  </tr>";
        }
    }
